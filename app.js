...
function renderJobs(jobs) {
  ...
  grid.innerHTML = jobs.map(j => {
    ...
    return `<div class="job-card" id="jc-${j.id}" onclick="toggleJob(this)" data-id="${j.id}">
      <div class="jc-main">
...
async function gerarMensagemRecrutador() {
  ...
  const prompt = `You are a senior recruiter at a top US tech company. You receive hundreds of LinkedIn messages from candidates every week. You know exactly which messages you actually read and respond to — and which ones you ignore in 2 seconds.

Write a LinkedIn direct message for a Brazilian professional who just applied for the job below. The message must feel like it was written by a real human, not generated by AI.

RULES FOR A MESSAGE THAT ACTUALLY GETS A RESPONSE:
- Maximum 80 words — recruiters ignore long messages
- Never start with "Hi, my name is" or "I just applied" as the opener — lead with value
- Reference ONE specific thing from the job description that connects to the candidate's experience
- Include ONE specific achievement with a number
- Show genuine interest in the company with ONE specific detail (product, recent news, mission)
- End with a low-pressure question, not "please review my application"
- Sound like a confident peer, not a desperate applicant
- Use [Recruiter Name] as placeholder

JOB: "${job.title}" at ${job.company_name}
JOB DESCRIPTION: ${descVaga}
CANDIDATE BACKGROUND: ${cvTexto}
CANDIDATE NAME: ${nome}
${linkedin ? `LINKEDIN: ${linkedin}` : ''}

Return ONLY the message text. No explanation, no subject line, no preamble.`;
  try {
